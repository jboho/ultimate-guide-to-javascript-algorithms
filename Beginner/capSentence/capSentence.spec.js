...
xtest('capitalizes the first letter of each word in a lowercase sentence', () => {
  expect(capSentence('i must confess, this is so much fun.')).toEqual(
    'I Must Confess, This Is So Much Fun.'
  );
});

xtest('capitalizes the first letter of each word in an uppercase sentence', () => {
  expect(capSentence('THIS ONE IS FOR SCOTCH.')).toEqual(
    'This One Is For Scotch.'
  );
});

xtest('capitalizes the first letter of each word in mixed cased sentences', () => {
